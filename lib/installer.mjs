...
export function install({sourceRoot, home, dryRun = false, force = false, withLocalHistory = false} = {}) {
  ...
  const autoPilotItem = items.find((item) => item.name === 'auto-pilot-skill')
  ...
  if (dryRun) {
    ...
    if (withLocalHistory) preview.push(installHistoryHooks({home: paths.home, installedSkill: autoPilotItem.destination, dryRun: true}))
    return {items: preview, backupRoot: null}
  }

  const replacements = items.filter((item) => item.status === 'new' || (item.status === 'conflict' && force))
  const hooksNeedBackup = withLocalHistory && pathExists(historyHooksPath(paths.home)) && historyHookStatus({home: paths.home, installedSkill: autoPilotItem.destination}) !== 'current'
  ...
  if (withLocalHistory) resultItems.push(installHistoryHooks({home: paths.home, installedSkill: autoPilotItem.destination, backupRoot}))
...
export function doctor({sourceRoot, home, withLocalHistory = false} = {}) {
  ...
  if (withLocalHistory) results.push({
    ...
    status: historyHookStatus({home: resolveHomeForDoctor(home), installedSkill: autoPilotItem.destination}),
  })
