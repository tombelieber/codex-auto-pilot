...
test('Auto Pilot is explicit-only and exposes a valid skill path', async () => {
  ...
  assert.equal(history.includes('HISTORY_SCHEMA_VERSION = 4'), true)
...
test('skill keeps owner-directed routing, terminal leaves, and a fresh release boundary', async () => {
  ...
  assert.match(skill, /A terminal `blocked`, `merged_main`, or `released` response permanently seals that release task/)
  assert.match(skill, /10-minute whole-task wall-clock release-control budget/)
  assert.match(skill, /do not repeat implementation, edit source, create commits or branches, open another PR/)
  ...
  assert.match(automaticPromotion, /Installed release-contract SHA-256/)
  assert.match(automaticPromotion, /Source receipt SHA-256/)
  assert.match(automaticPromotion, /If it has any completed terminal release turn, it is sealed/)
