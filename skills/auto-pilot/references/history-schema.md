...
`manifest.json` identifies the run mode, requested release continuation, collector and skill versions, the installed `SKILL.md` hash, the complete skill-bundle hash and per-file hashes, start/end times, model, effort, source offset, and retention state. `ship`, `--then-release`, and an unambiguous current implementation-then-release imperative record `continuation: release`; this is observational metadata, not standalone production authority. The manifest stores only a hash of the invocation prompt; the original prompt remains in the archived transcript.
